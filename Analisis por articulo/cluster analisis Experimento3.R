...
for(i in 1:kbest.p){
  pdf(paste(paste("D:\\Dropbox\\Año 5\\WikiAnalisis\\R scripts\\Experimento 3 cluster",i), ".pdf"))
  clusterx <- names(cboot$result$partition[cboot$result$partition==i])
  clusterx <- as.integer(clusterx)
  for(j in 1:length(clusterx)) {
    see <- data.frame(t(revisionindays[[clusterx[j]]][1,]))
    see$Date<-as.Date(rownames(see))
    colnames(see)<-c("Revisions","Date")

    print(ggplot(see) + geom_line(aes(Date, Revisions))  + xlab("") + ylab("Daily Views")+ggtitle(articlenames[[clusterx[j]]]))
