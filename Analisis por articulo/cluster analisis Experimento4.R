...
data<-df.pages.partes[,- c(4,5)]#arranco con el total de los datos
...
#K seleccionados para probar.  2,5,7,8,10,13
...
for(i in 1:kbest.p){
  pdf(paste(paste("D:\\Dropbox\\Año 5\\WikiAnalisis\\R scripts\\Experimento 4 cluster",i), ".pdf"))
  clusterx <- names(cboot$result$partition[cboot$result$partition==i])
  clusterx <- as.integer(clusterx)
  for(j in 1:length(clusterx)) {
    see <- as.data.frame(listapartes[df.pages.partes[clusterx[j],"idarticle"]][[1]][df.pages.partes[clusterx[j],"parte"]])
    see$Date<-as.Date(rownames(see))
    colnames(see)<-c("Revisions","Date")

    print(ggplot(see) + geom_line(aes(Date, Revisions))  + xlab("") + ylab("Daily Views")+ggtitle(df.pages.partes[clusterx[j],"articlename"]))
    #unlist(revisionindays[[1]][1,],use.names = FALSE)
    #print(ggplot(see)+geom_histogram(aes(x=Date),binwidth = 3, fill="gray")+ggtitle(articlenames[[clusterx[j]]]))
