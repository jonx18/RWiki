...
data<-df.articulos$johnny #arranco con el total de los datos
...
table_summary_clusters <- function(labels, k) {
  for(i in 1:k) {
    tmp<-summary(data[labels==i,])
    tmp<-gsub("Min.   :", "", tmp)
    tmp<-gsub("1st Qu.:", "", tmp)
    tmp<-gsub("Median :", "", tmp)
    tmp<-gsub("Mean   :", "", tmp)
...
#K seleccionados para probar. 10 , 11 , 12 , 13,14,15
...
table_summary_clusters(cboot$result$partition, kbest.p)
cboot$bootmean 
